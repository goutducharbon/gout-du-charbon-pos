...
export const bootstrapOwner = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    if (existing && existing.length > 0) throw new Error("bootstrap_already_done");
...
export const verifyPin = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (failed >= 5) throw new Error("locked_out");
...
export const getDashboardStats = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { data: orders, error } = await context.supabase
      .from("orders")
      .select("id, ticket_no, total, status, payments, created_at, lines")
    ...
    for (const r of paid) {
      const pays = Array.isArray(r.payments) ? (r.payments as Array<{ method?: string; amount?: number }>) : [];
      if (pays.length === 0) {
        byPayment["autre"] = (byPayment["autre"] ?? 0) + Number(r.total ?? 0);
      } else {
        for (const p of pays) {
          const m = String(p.method ?? "autre");
          byPayment[m] = (byPayment[m] ?? 0) + Number(p.amount ?? 0);
        }
      }
    }
    const topMap: Record<string, { name: string; qty: number; revenue: number }> = {};
    for (const r of paid) {
      const items = Array.isArray(r.lines) ? (r.lines as Array<Record<string, unknown>>) : [];
